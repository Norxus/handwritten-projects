...
use commands::{CommandRegistry, CommandManifestEntry, CommandSource};
...
impl UpstreamPaths {
    ...
    pub fn command_path(&self) -> PathBuf {
        self.repo_root.join("src/command.ts")
    }
...
fn imported_symbols(line: &str) -> Vec<String> {
    let Some(after_imported) = line.strip_prefix("imported ") else {
        return Vec::new();
    };
    let before_from = after_imported
        .split("from")
        .next()
        .unwrap_or_default()
        .trim();
    ...
    let first = before_from.split('.').next()?.to_string();
    if first.is_empty() {
        Vec::new()
    }else{
        vec![first.to_string()]
...
fn resolve_upstream_repo_root(primary_repo_root: &Path) -> PathBuf {
    ...
    candidates
        .into_iter()
        .find(|candidate| candidate.join("src/command.ts").is_file())
